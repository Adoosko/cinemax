...
export async function fetchCachedMovieBySlug(slug: string): Promise<Movie | null> {
  try {
    ...
    console.log(`Fetched movie data for slug ${slug}:`, {
      ...
      firstFewProps: data && typeof data === 'object' ? 
        Object.entries(data).slice(0, 3).map(([k, v]) => `${k}: ${typeof v}`) : []
    });
...
export async function fetchCachedMovieVideo(slug: string, useDirect: boolean = true): Promise<any | null> {
  try {
...
export async function CachedMovieVideoData({ slug, useDirect = true }: { slug: string, useDirect?: boolean }) {
