...
export function SeriesSidebarNavigation({
    ...
    const fetchWatchHistory = async () => {
      try {
        ...
        if (response.ok) {
          ...
          data.watchHistory.forEach((h: WatchHistoryItem) => {
            if (h.completed) {
  ...
  const SidebarContent = (
    <aside className="h-full w-full flex flex-col bg-black/80 backdrop-blur border-r border-white/10 sm:border-r-0">
      {/* Header */}
      ...
      <div className="flex-1 overflow-y-auto min-h-0 scrollbar-thin scrollbar-track-transparent scrollbar-thumb-white/20 hover:scrollbar-thumb-white/40">
        <div className="py-4">
      ...
      {/* Footer */}
      <div className="p-4 border-t border-white/10 text-white/40 text-xs text-center hidden sm:block flex-shrink-0">
        Watching: Episode{' '}
    ...
    <>
      ...
      <div className="sm:hidden">
        <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
          ...
          <SheetContent
            side="left"
            className="p-0 w-[320px] max-w-full bg-black/90 border-r border-white/10 overflow-hidden"
          >
