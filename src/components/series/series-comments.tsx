...
export function SeriesComments({ seriesSlug }: SeriesCommentsProps) {
  ...
  const fetchComments = async () => {
    try {
      const response = await fetch(`/api/series/${seriesSlug}/comments`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const optimisticComment: SeriesComment = {
      id: `temp-${Date.now()}`,
      userId: user!.id,
  ...
  const handleReply = async (parentId: string, e: React.FormEvent) => {
    ...
    const optimisticReply: SeriesComment = {
  ...
  const deleteComment = async (commentId: string) => {
    ...
    try {
      const response = await fetch(`/api/series/${seriesSlug}/comments?commentId=${commentId}`, {
        method: 'DELETE',
      });
        ...
        <Card className="bg-netflix-black/50 p-6">
          <div className="text-center py-4">
            ...
            <p className="text-netflix-text-gray mb-4">
              Sign in to share your thoughts about this series
            </p>
