...
import { AlertCircle, CheckCircle, Clock, Film, Play, Upload, X, Zap } from 'lucide-react';
...
export function EpisodeUpload({
  ...
  const uploadFile = async (quality: string, file: File) => {
    return new Promise<string>((resolve, reject) => {
      ...
      fetch('/api/admin/series/episode-upload', {
        method: 'POST',
        ...
        .then(({ uploadUrl, videoUrl, videoPath: path }) => {
          if (path && !videoPath) {
            setVideoPath(path);
          }
  ...
  const handleUploadAll = async () => {
    ...
    try {
      ...
      if (successfulUploads.length === results.length) {
        alert('All episode videos uploaded successfully!');
      } else {
        alert(`${successfulUploads.length}/${results.length} episode videos uploaded successfully`);
      }
    } catch (error) {
      ...
      >
        ...
        <div className="flex items-center justify-between p-8 border-b border-white/10">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-white">Upload Episode Video</h1>
                      ...
                      {videoPath ||
                        `${
                          seriesTitle
                            ...
                            .replace(/[^a-z0-9]+/g, '-')
                            .replace(/^-|-$/g, '') || seriesId
                        }/season-${seasonNumber}/episodes/${episodeNumber}`}
                      /{'{quality}'}.mp4
                    </div>
                  </div>
                </motion.div>
